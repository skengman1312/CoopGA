...
def plot_all_prevalence(data, title="", params=["N", "r", "dr", "mr", "cr"]):
    """
    function used to plot the mean of the allele prevalence across several simulation
    """
    # data = data.drop(columns=["N", "r", "RunId"])
...
def scatter3D_all(data, param1, param2, result, labels, all_params, title=""):
    """
    Function used to plot altruistic allele frequency (result) against two other parameters of user's choice.
    In the output each color will represent a different combination of parameters' values
    """
    ...
    # we want to plot just the value of the parameters in the last step of each iteration
    max_step = data["Step"].max()
    # results store now just the lines of the df where we had the last step of each iteration
    results = data[data["Step"] == max_step]
    ...
    maintitle = f"Kinship altruism {title}" if title else "Green Beard"
    plt.title(f"{maintitle}")
    filename = f"{title.replace(' ', '')}_results.png" if title else "results.png"
    plt.savefig(filename)
...
def plot_all_agents(data, title="", params=["N", "r", "dr", "mr", "cr"]):
    """
    function used to plot the number of agents with different genotypes across several simulation
    """
    # data = data.drop(columns=["N", "r", "RunId"])
    # masking the values of each iteration
    # creating a boolean mask for each iteration to be plotted
    ...
    # grouping all the lines in a single df without other data
    lines_true = pd.DataFrame(data={m: (data[msk[m]]["true beards fraction"]*data[msk[m]]["n_agents"]).reset_index(
        drop=True) for m in range(len(msk))})
    lines_suckers = pd.DataFrame(data={m: (data[msk[m]]["suckers fraction"]*data[msk[m]]["n_agents"]).reset_index(
        drop=True) for m in range(len(msk))})
    lines_impostors = pd.DataFrame(data={m: (data[msk[m]]["impostors fraction"]*data[msk[m]]["n_agents"]).reset_index(
        drop=True) for m in range(len(msk))})
    ...
    # setting plot limits
    max_agents = max(data["n_agents"])
    print(max_agents)
    plt.axis((0, ms, 0, max_agents))
...
if __name__ == "__main__":
    ...
    plot_all_agents(data, title="Green Beard linkage disequilibrium")
    #plot_all_agents(data1, title="Green Beard")
    ...
    #scatter3D(data1, param1="mr", param2="dr", result="altruistic fraction", labels=labels,
        #all_params=["N", "r", "dr", "mr"], title="scatter")
    ...
    #plot_all_prevalence(data, title="Green Beard linkage disequilibrium")
    ...
    labels = ["mutation rate", "death rate", "ending freq altruism"]
    # scatter3D_all(data, param1="mr", param2="dr", result="altruistic fraction", labels=labels,
        # all_params=["N", "r", "dr", "mr"], title="scatter")
    # scatter3D_all(data1, param1="mr", param2="dr", result="altruistic fraction", labels=labels,
        # all_params=["N", "r", "dr", "mr"], title="scatter")

    #scatter3D_all(multidata, param1="mr", param2="dr", result="altruistic fraction", labels=labels,
          #all_params=["N", "r", "dr", "mr"], title="scatter")
    #scatter3D_all(multidata1, param1="mr", param2="dr", result="altruistic fraction", labels=labels,
          #all_params=["N", "r", "dr", "mr"], title="scatter")
